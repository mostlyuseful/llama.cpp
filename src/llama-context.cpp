...
void llama_memory_breakdown_get(const struct llama_context * ctx,
    ...
    for (size_t i = 0; i < devices.size() && out_idx < *n_out; i++) {
        ...
        std::string full_name = name + " (" + desc + ")";

        size_t free, total;
        ggml_backend_dev_memory(dev, &free, &total);

        out[out_idx].model   = mb.model;
        out[out_idx].context = mb.context;
        out[out_idx].compute = mb.compute;
        out[out_idx].free    = free;
        out[out_idx].total   = total;
        out[out_idx].is_gpu  = true;
        snprintf(out[out_idx].name, sizeof(out[out_idx].name), "%s", full_name.c_str());
        out_idx++;
    }

    if (out_idx < *n_out) {
        out[out_idx].model   = mb_host.model;
        out[out_idx].context = mb_host.context;
        out[out_idx].compute = mb_host.compute;
        out[out_idx].free    = 0;
        out[out_idx].total   = 0;
        out[out_idx].is_gpu  = false;
        snprintf(out[out_idx].name, sizeof(out[out_idx].name), "Host");
        out_idx++;
    }
